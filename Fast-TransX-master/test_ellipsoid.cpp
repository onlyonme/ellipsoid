...
void prepare() {
    ...
    fclose(fin);
     fin = fopen((initPath + "C" + note + ".vec").c_str(), "r");
    for (long i = 0; i < relationTotal*2; i++) {
        long last = i * dimension;
        for (long j = 0; j < dimension; j++)
...
float clac_value(long eh,long et,long rel){
	...
	if((1-pow(k_h,-0.5))<0)
		...
		d_h=pow(1-pow(k_h,-0.5),2)*y_h;
	if((1-pow(k_t,-0.5))<0)
		d_t=0;
	else
		d_t=pow(1-pow(k_t,-0.5),2)*y_t;
        //cout<<"DD/res:"<<(d_h+d_t)/res<<endl;
	return res+(d_h+d_t)*150;
}
...
void* testMode(void *con) {
    ...
    for (long i = lef; i <= rig; i++) {
        ...
        float minimal = clac_value(h, t, r);
        long l_filter_s = 0;
        long l_s = 0;
        long r_filter_s = 0;
        long r_s = 0;
